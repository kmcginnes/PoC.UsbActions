...
namespace PoC.UsbActions
{
    ...
    public class RemoveDriveTools
    {
        ...
        public static bool RemoveDrive(string driveCharWithColon)
        {
            ...
            IntPtr hVolume = CreateFile(@"\\.\" + driveCharWithColon, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero,
        ...
        static long GetDeviceNumber(IntPtr handle)
        {
            ...
            try
            {
                DeviceIoControl(handle, IOCTL_STORAGE_GET_DEVICE_NUMBER, IntPtr.Zero, 0, buffer, size, out bytesReturned,
                    IntPtr.Zero);
            }
            finally
            {
            ...
            if (bytesReturned > 0)
            {
                STORAGE_DEVICE_NUMBER sdn =
                    (STORAGE_DEVICE_NUMBER) Marshal.PtrToStructure(buffer, typeof (STORAGE_DEVICE_NUMBER));
        ...
        static long GetDrivesDevInstByDeviceNumber(long DeviceNumber, DriveType DriveType, string dosDeviceName)
        {
            ...
            while (true)
            {
                ...
                IntPtr hDrive = CreateFile(devicePath, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, IntPtr.Zero, OPEN_EXISTING,
                    0, IntPtr.Zero);
